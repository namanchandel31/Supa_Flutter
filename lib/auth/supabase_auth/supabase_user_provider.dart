...
class SupabaseAuthSupabaseUser extends BaseAuthUser {
  SupabaseAuthSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> supabaseAuthSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SupabaseAuthSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
